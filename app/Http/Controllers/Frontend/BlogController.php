    ...
    public function getIndex(Request $request)
{
    ...
    if ($request->slug) {
        $blog_id = last(explode('-', $request->slug));
        ...
        return view('blogs.blog-single', compact('blog', 'previous', 'next', 'popular_tags', 'categories', 'related_news'));
    }

    $blogs = Blog::has('category')->orderBy('created_at', 'desc')->paginate(6);
    return view('blogs.index', compact('blogs', 'categories', 'popular_tags'));
}
