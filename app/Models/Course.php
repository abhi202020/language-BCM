...
class Course extends Model {
    ...
    protected $fillable = ['category_id', 'title', 'slug', 'description', 'price', 'stripe_id', 'course_image','course_video', 'start_date', 'published', 'free','featured', 'trending', 'popular', 'meta_title', 'meta_description', 'meta_keywords', 'expire_at','strike'];
...
//    protected $dates = ['expire_at'];
    ...
    // Set attribute to money format. @param $input
    public function setPriceAttribute($input) {
        $this->attributes['price'] = $input ? $input : null;
    }

    // Set attribute to date format. @param $input
    ...
    // Get attribute from date format. @param $input @return string
    ...
    public function scopeCanDisableCourse($query) {
        return $query->where(function($q){
                $q->whereNull('expire_at')->orWhereDate('expire_at', '>=', Carbon::now()->format('Y-m-d\TH:i'));
        });
    }
