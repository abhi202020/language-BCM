...
class GenerateSitemap extends Command
{
    ...
    public function handle()
    {
        ...
        for ($i = 1; $i <= $course_sitemap_ctr; $i++) {
            ...
            $sitemap->addSitemap(env('APP_URL') . '/sitemap-' . str_slug(config('app.name')) . '/' . 'course-list-' . $i . '.xml', date('Y-m-d\TH:i'));
        }
        ...
        for ($i = 1; $i <= $bundle_sitemap_ctr; $i++) {
            ...
            $sitemap->addSitemap(env('APP_URL') . '/sitemap-' . str_slug(config('app.name')) . '/' . 'bundle-list-' . $i . '.xml', date('Y-m-d\TH:i'));
        }
        $content = $sitemap->render('sitemapindex');
        Storage::disk('local')->put('sitemap-' . str_slug(config('app.name')) . '/sitemap-index.xml', $content->original);
        ...
        for ($i = 1; $i <= $blog_sitemap_ctr; $i++) {
            ...
            $sitemap->addSitemap(env('APP_URL') . '/sitemap-' . str_slug(config('app.name')) . '/' . 'blog-list-' . $i . '.xml', date('Y-m-d\TH:i'));
        }
        $content = $sitemap->render('sitemapindex');
        Storage::disk('local')->put('sitemap-' . str_slug(config('app.name')) . '/sitemap-index.xml', $content->original);
        ...
        Course::where('published', '=', 1)->where('published', '=', 1)->chunk($chunk_size, function ($courses_chunks) {
            ...
            foreach ($courses_chunks as $course) {
                ...
                $sitemap_courses->add(env('APP_URL') . '/course/' . $course->slug, date('Y-m-d\TH:i'), 0.8, 'daily', $images);
            }
        ...
        Bundle::where('published', '=', 1)->chunk($chunk_size, function ($bundles_chunks) {
            ...
            foreach ($bundles_chunks as $bundle) {
                ...
                $sitemap_bundles->add(env('APP_URL') . '/bundle/' . $bundle->slug, date('Y-m-d\TH:i'), 0.8, 'daily', $images);
            }
        ...
        Blog::chunk($chunk_size, function ($blogs_chunks) {
            ...
            foreach ($blogs_chunks as $blog) {
                ...
                $sitemap_blogs->add(env('APP_URL') . '/blog/' . $blog->slug . '-' . $blog->id, date('Y-m-d\TH:i'), 0.8, 'daily', $images);
            }
