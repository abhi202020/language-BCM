...
Route::group(['middleware' => 'role:student'], function () {
    Route::get('/book-slot/{slot_id}', 'App\Http\Controllers\LiveLessonSlotController@bookSlot')->name('book.slot');
});
...
Route::group(['namespace' => 'Backend', 'prefix' => 'admin', 'middleware' => config('menu.middleware')], function () {
    Route::post('add-custom-menu', 'MenuController@addcustommenu')->name('admin.add_custom_menu');

    Route::post('delete-item-menu', 'MenuController@deleteitemmenu')->name('admin.delete_item_menu');

    Route::post('delete-menug', 'MenuController@deletemenug')->name('admin.delete_menug'); // Updated route name

    Route::post('create-new-menu', 'MenuController@createnewmenu')->name('admin.create_new_menu'); // Updated route name

    Route::post('generate-menu-control', 'MenuController@generatemenucontrol')->name('admin.generate_menu_control'); // Updated route name

    Route::post('update-item', 'MenuController@updateitem')->name('admin.update_item'); // Updated route name

    Route::post('save-custom-menu', 'MenuController@saveCustomMenu')->name('admin.save_custom_menu'); // Updated route name

    Route::post('change-location', 'MenuController@updateLocation')->name('admin.update_location'); // Updated route name
});
...
Route::group(['prefix' => 'laravel-filemanager', 'middleware' => ['web', 'auth', 'role:teacher|administrator']], function () {
    ...
    // Assign unique names to specific routes generated by Lfm
    Route::name('laravel-filemanager.show')->get('/{view?}/{sub?}/{path?}', '\UniSharp\LaravelFilemanager\controllers\LfmController@show');
    Route::name('download-laravel-filemanager.download')->get('/get/{file}', '\UniSharp\LaravelFilemanager\controllers\DownloadController@get_file');
