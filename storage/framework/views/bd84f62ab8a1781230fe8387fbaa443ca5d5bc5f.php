...
<?php $__env->startSection('content'); ?>
    <div class="card message-box">
        <div class="card-header">
            <h3 class="page-title mb-0"><?php echo app('translator')->get('labels.backend.messages.title'); ?>
                <div class="float-right">
                    ...
                    <a href="<?php echo e(route('admin.messages')); ?>" class="d-lg-none text-decoration-none threads d-md-none">
        ...
        <div class="card-body">
            <div class="messaging">
                <div class="inbox_msg">
                    <div class="inbox_people <?php if(request('thread')): ?> d-none <?php endif; ?> d-md-block d-lg-block ">
                            ...
                            <?php if($threads->count() > 0): ?>
                                <?php $__currentLoopData = $threads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php if($item->latestMessage): ?>
                                        <a  href="<?php echo e(route('admin.messages').'?thread='.$item->id); ?>" class="<?php if($item->userUnreadMessagesCount(auth()->user()->id)): ?> unread
                                            <?php endif; ?>">
                                            <div data-thread="<?php echo e($item->id); ?>"
                                                ...
                                                <div class="chat_people">
                                                    ...
                                                    <div class="chat_ib">
                                                        <h5>
                                                            <?php echo e($item->participants()->with('user')->where('user_id','<>', auth()->user()->id)->first()->user->name); ?>
                    ...
                    <?php if(request()->has('thread')): ?>
                        <form method="post" class="compose-reply" action="<?php echo e(route('admin.messages.reply')); ?>">
                            <?php echo csrf_field(); ?>
                    ...
                    <?php endif; ?>
                    <form method="post" class="d-none compose-message" action="<?php echo e(route('admin.messages.send')); ?>">
                        <?php echo csrf_field(); ?>

                        <div class="headind_srch bg-dark">
                            <div class="chat_people header row">
                                <div class="col-12 col-lg-3">
                                    <p class="font-weight-bold text-white mb-0" style="line-height: 35px"><?php echo app('translator')->get('labels.backend.messages.select_recipients'); ?>:</p>
                                </div>
...
<?php echo $__env->make('backend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\language\resources\views\backend\messages\index-mobile.blade.php ENDPATH**/ ?>